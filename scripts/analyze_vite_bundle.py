...
# 读取 JSON 文件
json_path = Path('/tmp/bundle-analysis.json')
data = json.loads(json_path.read_text(encoding='utf-8'))

# 检查数据结构
print(f'数据类型: {type(data).__name__}')
print(f'主要属性: {list(data.keys()) if isinstance(data, dict) else "N/A"}')

# 查找 index-XFFXz8D0.js
...
index_chunk = find_chunk(data, 'index-XFFXz8D0.js')
...
if 'groups' in index_chunk:
    ...
    for i, (package_name, pkg_data) in enumerate(sorted_packages[:20], 1):
        ...
        print(f"   gzip: {pkg_data['totalGzipSize'] / 1024:.2f} KB")
