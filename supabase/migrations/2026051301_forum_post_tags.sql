-- 论坛标签系统：
-- 1) 为帖子增加可空 tag 字段，旧帖子保持 null 以兼容历史数据
-- 2) 列表 RPC 支持按标签过滤，并返回 tag
-- 3) 提供热门标签统计 RPC
...
alter table public.posts
  ...
  check (
    tag is null
    or tag in ('server', 'activity', 'daily', 'question')
  );

create index if not exists idx_posts_tag_status_created_at_id
...
as $$
  select p.tag, count(*)::bigint as post_count
   ...
   where p.status = 'approved'
     and p.tag in ('server', 'activity', 'daily', 'question')
   group by p.tag
   order by post_count desc, p.tag asc;
$$;
