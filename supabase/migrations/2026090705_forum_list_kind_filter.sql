-- 论坛信息流内容类型筛选下推到服务端。
-- 背景：官方卡（post_kind = news/activity，由 news/activities 触发器镜像生成）的
-- created_at 继承源内容旧日期，按 created_at desc 排序沉底；此前内容类型筛选只在前端
-- prepareForumPosts 做，单页 10 条混合内容里目标类型常常为 0，点「新闻」列表直接清空，
-- 无限滚动追拉多页也未必命中，表现为筛选器不工作。
-- 修复：list_forum_posts 增加 p_kind_filter，服务端按 post_kind 过滤后再分页。
-- 语义：'post'  = 论坛（普通帖 + 用户转发 repost）
--       'news'  = 官方新闻卡
--       'activity' = 官方活动卡
--       '' / null  = 不过滤（全部内容）
...
-- 新签名覆盖旧 8 参版本：避免 PostgREST 因两个函数都能匹配 8 参调用而报 PGRST202 歧义。
-- 旧客户端不传 p_kind_filter 时默认 null，行为与升级前完全一致。
drop function if exists public.list_forum_posts(integer, integer, text, uuid, boolean, text, text, uuid[]);

grant execute on function public.list_forum_posts(integer, integer, text, uuid, boolean, text, text, uuid[], text) to authenticated;
grant execute on function public.list_forum_posts(integer, integer, text, uuid, boolean, text, text, uuid[], text) to service_role;

comment on function public.list_forum_posts(integer, integer, text, uuid, boolean, text, text, uuid[], text) is
  '论坛列表查询；返回点赞状态、最多 4 张预览图、最多 3 条顶级评论预览，支持标签、搜索、排序、关注与内容类型（post/news/activity）筛选。';
