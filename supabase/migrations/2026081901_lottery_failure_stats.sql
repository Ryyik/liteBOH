-- Count losses for every existing lottery and expose an admin-readable summary.
-- This is reporting only and does not change historical draw behavior.

begin;

drop view if exists public.lottery_failure_stats;

create view public.lottery_failure_stats
...
with historical_lotteries as (
  ...
  select
    ...
    case when exists (
      ...
      where l.lottery_id = e.lottery_id
        and l.draw_no = h.latest_draw_no
...
comment on view public.lottery_failure_stats is
  '按用户汇总所有已结束抽奖的参与、中奖、未中奖和当前连续未中奖次数';
