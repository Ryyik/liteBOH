-- 修复 list_forum_posts 在运行时的歧义列错误：
-- 42702: column reference "status" is ambiguous
-- 原因：PL/pgSQL returns table 输出列名 status 与子查询 where status = 'approved' 发生歧义。
...
begin
  ...
  post_stats as (
    ...
    from base_posts bp
    left join (
      select cmt.post_id, count(*)::bigint as comment_count
      from public.comments cmt
      where cmt.status = 'approved'
      group by cmt.post_id
    ) c on c.post_id = bp.id
    left join (
      select lk.post_id, count(*)::bigint as like_count
