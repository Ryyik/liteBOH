-- 头像框全站可见（Phase 2 数据链路）：
-- 1) profiles 记录佩戴框 URL（相对路径，如 /avatars/frames/orange-cat-frame.png）
-- 2) list_forum_posts 带出作者框字段（帖子作者 + 预览回复作者）

begin;

alter table public.profiles add column if not exists avatar_frame_url text;
...
comment on function public.list_forum_posts(integer, integer, text, uuid, boolean, text, text, uuid[]) is
  '论坛列表查询；返回点赞状态、最多 4 张预览图、最多 3 条顶级评论预览，支持标签、搜索、排序与关注筛选。2026091106 增加作者头像框字段。';
