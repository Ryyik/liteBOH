-- =============================================
-- 订阅发放：已有订阅名单 + 跳过已有订阅用户
-- 1) admin_batch_grant_subscriptions 增加 p_skip_existing 参数
--    - 为 true 时排除已有「同层级生效订阅」的用户，返回 skipped 数量
-- 2) admin_list_existing_subscribers：列出指定层级已有生效订阅的用户名单
-- =============================================

begin;

-- 移除旧签名，避免重载歧义
drop function if exists public.admin_batch_grant_subscriptions(uuid[], text, text, text, integer, integer, timestamptz, timestamptz, text, jsonb);
...
revoke all on function public.admin_batch_grant_subscriptions(uuid[], text, text, text, integer, integer, timestamptz, timestamptz, text, jsonb, boolean) from public;
grant execute on function public.admin_batch_grant_subscriptions(uuid[], text, text, text, integer, integer, timestamptz, timestamptz, text, jsonb, boolean) to authenticated;
grant execute on function public.admin_batch_grant_subscriptions(uuid[], text, text, text, integer, integer, timestamptz, timestamptz, text, jsonb, boolean) to service_role;
