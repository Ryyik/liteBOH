...
<style scoped>
.points-card { position: relative; isolation: isolate; display: block; width: 100%; aspect-ratio: 8 / 5; overflow: hidden; border: 1px solid rgba(18, 34, 51, .13); border-radius: 18px; background: #edf2f4; color: #122233; box-shadow: 0 16px 32px rgba(27, 49, 69, .13); text-align: left; }
.points-card.is-interactive { cursor: pointer; }
.points-card.is-blank { background: #edf2f4; }
.points-card.is-cats { background: #fff; border-color: rgba(181, 117, 135, .22); color: #3c3437; box-shadow: 0 16px 32px rgba(169, 104, 126, .13); }
.points-card-custom-image { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; z-index: 0; }
.points-card-scrim { position: absolute; inset: 0; z-index: 1; background: rgba(247, 251, 252, .14); }
.points-card.has-custom-image .points-card-scrim { background: rgba(8, 19, 28, .38); }
...
.points-card-content { position: relative; z-index: 2; display: flex; min-height: 100%; flex-direction: column; padding: clamp(16px, 5%, 24px); }
.points-card-topline, .points-card-footer { display: flex; align-items: center; justify-content: space-between; gap: 12px; font-size: 12px; font-weight: 720; letter-spacing: 0; }
.points-card-points { margin-top: auto; font-size: clamp(29px, 9vw, 44px); font-weight: 780; line-height: 1; font-variant-numeric: tabular-nums; }
.points-card-label { margin-top: 5px; font-size: 12px; opacity: .72; }.points-card-footer { margin-top: auto; font-size: 11px; opacity: .7; }
.points-card-footer-actions { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }.points-card-sponsor { min-height: 28px; padding: 0 11px; border: 1px solid rgba(18, 34, 51, .18); border-radius: 999px; background: rgba(255,255,255,.76); color: inherit; font: inherit; font-size: 12px; font-weight: 720; cursor: pointer; }.points-card.has-custom-image .points-card-sponsor { border-color: rgba(255,255,255,.45); background: rgba(0,0,0,.18); color: #fff; }.points-card-sponsor:hover { background: #fff; }.points-card.has-custom-image .points-card-sponsor:hover { background: rgba(0,0,0,.34); }
.points-card.has-custom-image { color: #fff; border-color: rgba(255,255,255,.35); }.points-card.has-custom-image .points-card-label, .points-card.has-custom-image .points-card-footer { opacity: .88; }
.points-card.is-interactive:hover { transform: translateY(-2px); box-shadow: 0 20px 38px rgba(27, 49, 69, .18); }.points-card.is-interactive:focus-visible { outline: 3px solid #2d8c7c; outline-offset: 3px; }
.points-card.is-compact { max-width: 368px; aspect-ratio: 2.08 / 1; border-radius: 14px; }.points-card.is-compact .points-card-content { padding: 15px 17px; }.points-card.is-compact .points-card-points { font-size: 28px; }
@media (max-width: 420px) { .points-card-cat:nth-child(3) { width: 19%; }.points-card-cat:nth-child(4) { width: 17%; }.points-card-cat:nth-child(9), .points-card-cat:nth-child(10) { width: 12%; }.points-card-footer-actions { gap: 6px; }.points-card-sponsor { padding: 0 9px; } }
